    ...
    module Urls
      private
        ...
        def child_url_parts(action = nil, child = nil, url_params = {})
          [action] + contexts_url_parts + [singleton? ? model_name : [model_name, current_object_to_use(url_params)], [child.is_a?(Symbol) ? child : child.class.name.underscore.to_sym, child], url_params]
        end
        
        def child_collection_url_parts(action = nil, child_name = nil, url_params = {})
          [action] + contexts_url_parts + [singleton? ? model_name : [model_name, current_object_to_use(url_params)], child_name, url_params]
        end
